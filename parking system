...
# Auto class inherited from Vehicle class for license plate, company name and their type
class Auto(Vehicle):
    def __init__(self, licensePlate, companyName):
        Vehicle.__init__(self, licensePlate, companyName, VehicleType.AUTO)
...
class ParkingLot:
    ...
    # This function inserts a vehicle accordingly, also takes care of what company vehicle it is.
    ...
    # This function exits a vehicle 'C' in a level 'm'.
    def leaveOperation(self, vehicle):
        for level in self.levels:
            if level.remove(vehicle):
                return True

    # This function allows the user to view the list of vehicles parked for a particular company.
    def companyParked(self, companyName):
        all_vehicles = []
        for level in self.levels:
